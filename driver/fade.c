  ...
  case GenericEvent:
    {
      ...
      return (re->evtype == XI_RawKeyPress ||
...
static Bool
user_active_p (XtAppContext app, Display *dpy)
{
  XEvent event;
  XtInputMask m;
...
# endif
   ...
   */
  if (XCheckIfEvent (dpy, &event, &user_event_p, 0))
    {
      if (verbose_p > 1)
        {
          XIRawEvent *re = 0;
...
{
  XEvent event;
  while (XCheckIfEvent (dpy, &event, &user_event_p, 0))
    if (verbose_p > 1)
      {
        XIRawEvent *re = 0;
        if (event.xany.type == GenericEvent && !event.xcookie.data)
