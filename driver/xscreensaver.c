...
static int
grab_mouse (Screen *screen)
{
  Display *dpy = DisplayOfScreen (screen);
  Window w = RootWindowOfScreen (screen);
  Cursor cursor = None;    /* Set by xscreensaver-gfx and xscreensaver-auth */
  ...
  if (debug_p)		/* Passwords show up in plaintext! */
    {
      ...
      c[n] = 0;
      fprintf (stderr, "%s: %s    %02x %02x %s \"%s\"\n", blurb(),
               (x11_p
...
static void
print_xinput_event (Display *dpy, XEvent *xev, int x, int y, const char *desc)
{
  ...
  case XI_RawMotion:
    if (verbose_p > 1)
      fprintf (stderr, "%s: XI_RawMotion       %4d, %-4d  %7.02f, %-7.02f%s\n",
               blurb(), x, y, re->raw_values[0], re->raw_values[1],
               (desc ? desc : ""));
    break;
      ...
      while (XPending (dpy))
        {
                  ...
                  if (verbose_p)
                    {
                      ...
                      char *tstr = type ? XGetAtomName (dpy, type) : 0;
                      char *name = msg  ? XGetAtomName (dpy, msg)  : 0;
                      fprintf (stderr,
          ...
          case XI_RawButtonRelease:
            if (verbose_p > 1 ||
                (verbose_p && now - active_at > 1))
              print_xinput_event (dpy, &xev, 0, 0, 0);
            active_at = now;
            break;
              ...
              if (secs >= 1)
                {
                  ...
                  if (verbose_p > 1 ||
                    ...
                    print_xinput_event (dpy, &xev, root_x, root_y, 
                                        (ignored_p ? " ignored" : ""));
                }
            }
            break;
