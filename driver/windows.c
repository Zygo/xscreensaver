...
initialize_screensaver_window_1 (saver_screen_info *ssi)
{
  ...
  if (!ssi->screensaver_window)
	...
	XCreateWindow (si->dpy, RootWindowOfScreen (ssi->screen),
      ...
      XStoreName (si->dpy, ssi->screensaver_window, "XScreenSaver");
...
static void 
raise_windows (saver_info *si, Bool inhibit_fade, Bool dont_clear)
{
  saver_preferences *p = &si->prefs;
  int i;
                     ...
                     const char *name, const char *error)
{
  ...
  window = ssi->error_dialog =
    XCreateWindow (si->dpy, RootWindowOfScreen (ssi->screen),
                   ...
                   attrmask, &attrs);
  XStoreName (si->dpy, window, "XScreenSaver");
...
watchdog_timer (XtPointer closure, XtIntervalId *id)
{
  ...
  any_running_p = any_screenhacks_running_p (si);

  if (si->prefs.debug_p)
    fprintf (stderr, "%s: watchdog timer raising %sscreen\n",
             blurb(), (any_running_p ? "" : "and clearing "));

  raise_windows (si, True, any_running_p);

  if (any_running_p && !monitor_powered_on_p (si->dpy))
    {
      int i;
