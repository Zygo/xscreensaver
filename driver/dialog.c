...
struct window_state {
  ...
  char censored_passwd[MAX_PASSWD_CHARS];
                  ...
                  int left, int right, int y, Bool clear_p)
{
  ...
  case BUTTON:			/* box is fixed width at 1/3, text centered */
    align = CENTER;
    xpad = 0;
    ypad = ws->shadow_width + line->font->ascent / 2;
  ...
  case TEXT_RO:
    ...
    XftTextExtentsUtf8_multi (ws->dpy, line->font, (FcChar8 *) text,
                              strlen(text), &overall);
    if (overall.width >= w/2 - ws->shadow_width)
      {
        align = RIGHT;
        left = right - w/2;
      }
  ...
  rect.x = MAX (oleft, MAX (left, tleft));
  rect.y = y + ypad;
  rect.width = MIN (oright, right) - rect.x;
  rect.height = nlines * (line->font->ascent + line->font->descent) +
    ws->shadow_width;
...
window_init (Widget root_widget, Bool splash_p)
{
  ...
  cmap = XCreateColormap (dpy, RootWindowOfScreen (screen),  /* Old skool */
  ...
  ws->passwd_font   = get_font (ws, "passwdFont");
...
# endif
  ...
  /* Load the logo pixmap, based on font size */
  {
    ...
    ws->logo_pixmap = xscreensaver_logo (screen, visual, root, cmap,
                                         ws->background, 
  ...
  XSetWindowColormap (ws->dpy, ws->window, cmap);
  ...
  /* Even override-redirect windows should identify themselves. */
  {
    XClassHint class_hints;
    class_hints.res_name  = "xscreensaver";
    class_hints.res_class = "XScreenSaver";
    XStoreName (ws->dpy, ws->window, "XScreenSaver");
    XSetClassHint (ws->dpy, ws->window, &class_hints);
    XChangeProperty (ws->dpy, ws->window, XA_WM_COMMAND, XA_STRING,
 ...
 */
static void
window_draw (window_state *ws, Bool force_raise_p)
{
  Display *dpy = ws->dpy;
  ...
  text_left = (thermo_x + ws->internal_padding +
               (ws->thermo_width
                ...
                : 0));
  text_right = text_left + ws->heading_font->ascent * 20;
...
# define BLANK_LINE                    \
      ...
      case AUTH_MSGTYPE_PROMPT_ECHO:
        ...
        lines[i].text    = (ws->auth_state == AUTH_FINISHED
                            ...
                            : ws->show_stars_p
                              ? ws->censored_passwd
                              : "");
        lines[i].font    = (ws->auth_state == AUTH_FINISHED
                            ? ws->body_font
                            : ws->passwd_font);
        lines[i].fg      = ws->xft_text_foreground;
        lines[i].bg      = ws->passwd_background;
  ...
  /* Now that everything has been rendered into the pixmap, reshape
     the window and copy the pixmap to it.  This double-buffering is
     to prevent flicker.
   */
...

  {
    ...
    if (! (xgwa.x == ws->x && xgwa.y == ws->y &&
           xgwa.width == window_width && xgwa.height == window_height))
      {
        XWindowChanges wc;
        wc.x = ws->x;
        wc.y = ws->y;
        ...
        XInstallColormap (ws->dpy, xgwa.colormap);
        XSync (ws->dpy, False);
      }
    else if (force_raise_p)
      {
        if (verbose_p)
          fprintf (stderr, "%s: raising window\n", blurb());
        XMapRaised (ws->dpy, ws->window);
        XSync (ws->dpy, False);
      }
  }
...
destroy_window (window_state *ws)
{
  XWindowAttributes xgwa;
  Colormap cmap = 0;
  Pixel black = BlackPixelOfScreen (ws->screen);
  Pixel white = WhitePixelOfScreen (ws->screen);
  XEvent event;
  ...
  if (ws->passwd_font)   XftFontClose (ws->dpy, ws->passwd_font);
  ...
  if (ws->foreground != black && ws->foreground != white)
    XFreeColors (ws->dpy, cmap, &ws->foreground, 1, 0L);
  if (ws->background != black && ws->background != white)
    XFreeColors (ws->dpy, cmap, &ws->background, 1, 0L);
  if (!(ws->button_background == black || ws->button_background == white))
    XFreeColors (ws->dpy, cmap, &ws->button_background, 1, 0L);
  if (ws->passwd_background != black && ws->passwd_background != white)
    XFreeColors (ws->dpy, cmap, &ws->passwd_background, 1, 0L);
  if (ws->thermo_foreground != black && ws->thermo_foreground != white)
    XFreeColors (ws->dpy, cmap, &ws->thermo_foreground, 1, 0L);
  if (ws->thermo_background != black && ws->thermo_background != white)
    XFreeColors (ws->dpy, cmap, &ws->thermo_background, 1, 0L);
  if (ws->logo_background != black && ws->logo_background != white)
    XFreeColors (ws->dpy, cmap, &ws->logo_background, 1, 0L);
  if (ws->shadow_top != black && ws->shadow_top != white)
    XFreeColors (ws->dpy, cmap, &ws->shadow_top, 1, 0L);
  if (ws->shadow_bottom != black && ws->shadow_bottom != white)
    XFreeColors (ws->dpy, cmap, &ws->shadow_bottom, 1, 0L);
  ...
  if (ws->logo_pixels)
    {
      if (ws->logo_npixels)
        XFreeColors (ws->dpy, cmap, ws->logo_pixels, ws->logo_npixels, 0L);
      free (ws->logo_pixels);
      ws->logo_pixels = 0;
      ws->logo_npixels = 0;
    }
  ...
  while (ws->auth_state == AUTH_READ ||
         ws->auth_state == AUTH_NOTIFY)
    {
      ...
      if (xev.xcookie.type == GenericEvent &&
          ...
          XGetEventData (ws->dpy, &xev.xcookie))
        {
          XEvent ev2;
          Bool ok =
      ...
      case VisibilityNotify:
        if (xev.xvisibility.state == VisibilityPartiallyObscured ||
            xev.xvisibility.state == VisibilityFullyObscured)
          raise_p = True;
        refresh_p = True;
        if (verbose_p > 1)
      ...
      /* If we raise our window when it is already on top, that generates
         *another* ConfigureNotify event on the root window, causing a loop.
         Only raise it when necessary. */
      if (raise_p && !window_occluded_p (ws->dpy, ws->window))
        {
          raise_p = False;
          if (verbose_p > 1)
            fprintf (stderr, "%s: already on top, not raising\n", blurb());
        }
    }
