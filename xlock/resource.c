...
static argtype modevars[] =
{
	...
	{(caddr_t *) & count, "batchcount", "BatchCount", DEF_COUNT, t_Int},
...
checkSpecialArgs(int argc, char **argv)
{
	...
	for (i = 0; i < argc; i++) {
		...
		{
			char       *depreciated_args[] =
			{"-v", "-imagefile", "-mfont"};
			char       *current_args[] =
			{"-verbose", "-bitmap", "-messagefont"};
			int         j;

			for (j = 0; j < (sizeof (current_args)) / sizeof (*(current_args)); j++)
...
#endif
	XrmParseCommand(&nameDB, nameTable, 1, "xlock",
			&argc, argv);
	GetResource(nameDB, "xlock", classname, "name", "Name", t_String,
