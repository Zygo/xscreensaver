...
static const char sccsid[] = "@(#)xlock.c	4.04 97/07/28 xlockmore";
...
GrabKeyboardAndMouse(void)
{
	...
	status = XGrabKeyboard(dsp, Scr[0].window, True,
	...
	status = XGrabPointer(dsp, Scr[0].window, True, AllPointerEventMask,
...
XChangeGrabbedCursor(Cursor cursor)
{
	if (!debug && grabmouse && !inwindow && !inroot)
		(void) XGrabPointer(dsp, Scr[0].window, True, AllPointerEventMask,
...
mode_info(int scrn, Window window, int iconic)
{
	...
	MI_WIN_DEPTH(mi) = Scr[scrn].depth;
	MI_VISUAL(mi) = Scr[scrn].visual;
...
finish(void)
{
	for (screen = 0; screen < screens; screen++) {
		if (Scr[screen].window != None) {
			release_last_mode(mode_info(screen, Scr[screen].window, False));
		}
		if (Scr[screen].icon != None) {
			release_last_mode(mode_info(screen, Scr[screen].icon, True));
		}
	}
...
#endif
				       len / 60, len % 60);
		putText(Scr[scr].window, Scr[scr].textgc, buf, False, left, &x, &y);
	}
#ifdef USE_BUTTON_LOGOUT
	if (enable_button) {
		if (logoutButton > len) {
			...
			putText(Scr[scr].window, Scr[scr].textgc,
#ifdef NL
				"Schop me eruit                                               \n",
#else
#ifdef DE
			...
			if (!made_button || (isnew && tried_logout)) {
				...
				XSetForeground(dsp, Scr[scr].gc, Scr[scr].bgcol);
				XFillRectangle(dsp, Scr[scr].window, Scr[scr].gc, left, y,
					       xgwa.width - left,
				     5 + 2 * (font->ascent + font->descent));
				XSetForeground(dsp, Scr[scr].gc, Scr[scr].fgcol);

				if (tried_logout) {
					putText(Scr[scr].window, Scr[scr].textgc, logoutFailedString,
						True, left, &x, &y);
				} else {
					...
					XDrawString(dsp, Scr[scr].button, Scr[scr].gc, 0, font->ascent + 1,
						    buf, strlen(buf));
					XSetForeground(dsp, Scr[scr].gc, Scr[scr].fgcol);
					y += 5 + 2 * font->ascent + font->descent;
					putText(Scr[scr].window, Scr[scr].textgc, logoutButtonHelp,
						False, left, &x, &y);
				}
				y = ysave;
				x = left;
	...
	for (screen = 0; screen < screens; screen++)
		if (thisscreen == screen) {
			call_init_hook((LockStruct *) NULL,
				  mode_info(screen, Scr[screen].icon, True));
		} else {
			call_init_hook((LockStruct *) NULL,
			       mode_info(screen, Scr[screen].window, False));
		}
	...
	for (;;) {
		...
		switch (event.type) {
			case KeyPress:
				...
				XSetForeground(dsp, Scr[screen].gc, Scr[screen].bgcol);
				if (echokeys) {
					...
					XDrawString(dsp, Scr[screen].window, Scr[screen].textgc,
			...
			case ButtonPress:
				...
				if (((XButtonEvent *) & event)->window == Scr[screen].button) {
					...
					XSetForeground(dsp, Scr[screen].gc, Scr[screen].fgcol);
...
getPassword(void)
{
	...
	putText(Scr[screen].window, Scr[screen].textgc, MI_NAME(mi), True, left, &x, &y);
	putText(Scr[screen].window, Scr[screen].textgc, ": ", True, left, &x, &y);
	putText(Scr[screen].window, Scr[screen].textgc, MI_DESC(mi), False, left, &x, &y);
	putText(Scr[screen].window, Scr[screen].textgc, "\n", False, left, &x, &y);

	x = left = Scr[screen].iconpos.x + iconwidth + font->max_bounds.width;
	y = Scr[screen].iconpos.y + font->ascent;

	hostbuf = (char *) malloc(strlen(user) + strlen(hostname) + 2);
	(void) sprintf(hostbuf, "%s@%s", user, hostname);

	putText(Scr[screen].window, Scr[screen].textgc, text_user, True, left, &x, &y);
	putText(Scr[screen].window, Scr[screen].textgc, hostbuf, False, left, &x, &y);
	if (displayname && displayname[0] != ':') {
		char       *colon = (char *) strchr(displayname, ':');
		int         n = colon - displayname;

		(void) strncpy(hostbuf, displayname, n);
		hostbuf[n] = '\0';
		if (remote && n
		    && strcmp(hostbuf, "unix") != 0
		    && strcmp(hostbuf, "localhost") != 0
		    && strcmp(hostbuf, hostname) != 0) {
			putText(Scr[screen].window, Scr[screen].textgc, "  Display: ", True, left, &x, &y);
			putText(Scr[screen].window, Scr[screen].textgc, hostbuf, False, left, &x, &y);
		}
	}
	...
	putText(Scr[screen].window, Scr[screen].textgc, text_pass, True, left, &x, &y);
	putText(Scr[screen].window, Scr[screen].textgc, " ", False, left, &x, &y);
	...
	x = left = Scr[screen].iconpos.x;
	putText(Scr[screen].window, Scr[screen].textgc, text_info, False, left, &x, &y);
	putText(Scr[screen].window, Scr[screen].textgc, "\n", False, left, &x, &y);
	...
	if (*message) {
		for (msgp = message; *msgp; ++msgp) {
			...
			XDrawString(dsp, Scr[screen].window, Scr[screen].msgtextgc,
...
justDisplay(void)
{
	...
	for (screen = 0; screen < screens; screen++) {
		call_init_hook((LockStruct *) NULL, mode_info(screen, Scr[screen].window, False));
	}
				...
				if (event.xbutton.button == Button2) {
					...
					for (screen = 0; screen < screens; screen++) {
						call_change_hook((LockStruct *) NULL,
								 mode_info(screen, Scr[screen].window,
								     False));
					}
				} else {
...
sigcatch(int signum)
{
	ModeInfo   *mi = mode_info(0, Scr[0].window, False);
 ...
 */
	...
	defaultVisualInfo(dsp, screen);
      ...
      XCreateColormap(dsp, Scr[screen].root, Scr[screen].visual, AllocNone);
		...
		Scr[screen].fgcol = allocPixel(dsp, cmap, foreground, "Black");
		Scr[screen].bgcol = allocPixel(dsp, cmap, background, "White");
		if (use3d) {
			XColor      C;
			...
			if (!install || !XAllocColorCells(dsp, cmap, False, planemasks, 2, pixels,
				...
				/* color overlapping */
				Scr[screen].nonecol = allocPixel(dsp, cmap, none3d, DEF_NONE3D);
				Scr[screen].rightcol = allocPixel(dsp, cmap, right3d, DEF_RIGHT3D);
				Scr[screen].leftcol = allocPixel(dsp, cmap, left3d, DEF_LEFT3D);
				Scr[screen].bothcol = allocPixel(dsp, cmap, both3d, DEF_BOTH3D);
...
#endif
				Scr[screen].leftcol = C.pixel = pixels[0] | planemasks[1];
				XStoreColor(dsp, cmap, &C);
...
#endif
				Scr[screen].bothcol = C.pixel = pixels[0] | planemasks[0] | planemasks[1];
				XStoreColor(dsp, cmap, &C);
			}
...
#define CWMASK (((debug||inwindow||inroot)? 0 : CWOverrideRedirect) | CWBackPixel | CWBorderPixel | CWEventMask)
		...
		if (inroot) {
			...
			XSelectInput(dsp, Scr[screen].window, VisibilityChangeMask | ExposureMask);
		} else
#endif
#ifdef USE_DTSAVER
	if (!dtsaver)
...
#define XLOCKWIN_VISUAL CopyFromParent
		...
		fixColormap(dsp, Scr[screen].window, screen, saturation,
...
#define CIMASK CWBorderPixel | CWBackPixel | CWEventMask
		...
		xgcv.foreground = Scr[screen].fgcol;
		xgcv.background = Scr[screen].bgcol;
