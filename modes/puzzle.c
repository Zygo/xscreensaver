...
static const char sccsid[] = "@(#)puzzle.c        4.04 97/07/28 xlockmore";
...
cartoon.c has the same disease.
 */
...
setupmovedelta(ModeInfo * mi)
{
	...
	if (pp->box) {
		...
		if (XQueryTree(display, window, &root, &parent,
			...
			/*
			 * X Error of failed request:  Bad Window has been reported.
			 * It is probably on the next line or the line after. */
