 ...
 *   state zero were not bothered with since a calloc was used to set
 *   non-explicit rules to zero.  This allows the compile-time option
 *   RAND_RULES to work here (at least to generation 540).)
 *   Also added compile-time option DELAYDEBUGLOOP for debugging life form.
 *   This also turns off the random initial direction of the loop.
 *   Set DELAYDEBUGLOOP to be 10 and use with something like this:
 ...
 *   Automata Physica 10D 135-144 1984, also used wire.c as a guide.
 */
