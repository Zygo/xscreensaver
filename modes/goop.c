...
make_blob(blob * b, int maxx, int maxy, int size)
{
	...
	b->elasticity = SCALE * 1.8;	/* elasticity init */
	b->max_velocity = SCALE * 1.2;	/* max_velocity init */
	...
	make_spline(&(b->spline), b->npoints);
...
#else
	if (fill_p)
		XFillPolygon(display, drawable, gc, b->spline.points, b->spline.n_points,
			     Nonconvex, CoordModeOrigin);
	else
#endif
		XDrawLines(display, drawable, gc, b->spline.points, b->spline.n_points,
			   CoordModeOrigin);
}
...
make_layer(ModeInfo * mi, layer * l, int nblobs)
{
	...
	for (i = 0; i < l->nblobs; i++)
		make_blob(&(l->blobs[i]), width, height,
			  (LRAND() % (blob_max - blob_min)) + blob_min);
...
free_goop(Display * display, goopstruct * gp)
{
	...
	if (gp->layers) {
		for (l = 0; l < gp->nlayers; l++) {
			if (gp->layers[l].blobs) {
				...
				for (b = 0; b < gp->layers[l].nblobs; b++) {
					...
					free_spline(&(gp->layers[l].blobs[b].spline));
				}
		...
		if (gp->window_gc != None) {
			XFreeGC(display, gp->window_gc);
			gp->window_gc = None;
		}
...
init_goop(ModeInfo * mi)
{
	...
	if (gp->nlayers <= 0)
		gp->nlayers = (LRAND() % (MI_DEPTH(mi) - 2)) + 2;
	...
	gp->window_gc = XCreateGC(display, window, GCForeground | GCBackground, &gcv);
}
 ...
 */
			...
			XCopyArea(display, gp->pixmap, window, gp->window_gc, 0, 0,
		...
		case xor:
			...
			for (i = 0; i < gp->nlayers; i++)
				draw_layer_blobs(display, gp->pixmap, gp->pixmap_gc,
				     &(gp->layers[i]), gp->width, gp->height,
						 (gp->mode != outline));
			XCopyPlane(display, gp->pixmap, window, gp->window_gc, 0, 0,
