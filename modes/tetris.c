...
static void
drawBox(ModeInfo * mi, int cid, int i, int j)
{
  if (j < WELL_DEPTH) {
    ...
    drawSquare(mi, 15, cid, base_i, base_j);
  }
}
    ...
    for (j = 0; j < tp->curPolyomino.size; j++) {
      if (polytris[(int) tp->bonusNow].polyomino[tp->curPolyomino.polyomino_number].shape[j * tp->curPolyomino.size + i]) {
        if (tp->well) {
          ...
          drawBox(mi, tp->curPolyomino.color_number, xi, yj);
        } else {
...
drawPolyominoDiff(ModeInfo *mi, thing_t * old)
{
  ...
  /* Erase first or will have invisible squares when folded down a corner */
  for (i = 0; i < tp->curPolyomino.size; i++)
    for (j = 0; j < tp->curPolyomino.size; j++) {
