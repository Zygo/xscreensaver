...
static const char sccsid[] = "@(#)skewb.c	4.17 2000/04/27 xlockmore";
...
draw_stickerless_cubit(skewbstruct * sp)
{
	if (!sp->AreObjectsDefined[ObjCubit]) {
		...
		glEnd();
		glEndList();
		sp->AreObjectsDefined[ObjCubit] = 1;
#ifdef DEBUG_LISTS
...
draw_stickerless_facit(skewbstruct * sp)
{
	if (!sp->AreObjectsDefined[ObjFacit]) {
		...
		glEnd();
		glEndList();
		sp->AreObjectsDefined[ObjFacit] = 1;
#ifdef DEBUG_LISTS
...
shuffle(ModeInfo * mi)
{
	...
	if ((sp->storedmoves) && (sp->moves == NULL))
		if ((sp->moves =
		     (SkewbMove *) calloc(sp->storedmoves + 1,
				sizeof (SkewbMove))) == NULL)
			(void) fprintf(stderr,
			"Could not allocate memory for skewb move buffer\n");
...
reshape(ModeInfo * mi, int width, int height)
{
	...
	sp->AreObjectsDefined[ObjCubit] = 0;
}
