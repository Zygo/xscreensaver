...
static const char sccsid[] = "@(#)ant.c	4.07 97/11/24 xlockmore";
...
#if 1				/* #ifndef XSCREENSAVER_PREF */
/*-
 * jwz sez: this looks like crap 
 * dab says this is illogical; only squares (neighbors == 4) were in the
 *   original description of ants, in addition, removing them here is
 *   incorrect.
 */
	} else if (ap->neighbors == 4 || ap->neighbors == 8) {
		XFillRectangle(MI_DISPLAY(mi), MI_WINDOW(mi), gc,
...
#ifdef XSCREENSAVER_PREF
/* jwz sez: small sizes look like crap, dab says they don't */
	if (size < 0)
		size = NRAND(-size) + 1;
	if (size < 5)
		size += 5;
