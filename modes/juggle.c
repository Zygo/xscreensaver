...
#define DEF_PATTERNTRACE 0 /* No trace */
#define DEF_SOLID 0 /* Not solid */

static char *pattern;
static int patterntrace;
static Bool solid;
...
static XrmOptionDescRec opts[] =
{
  ...
  {(char* ) "-patterntrace", (char *) ".juggle.patterntrace",
   XrmoptionSepArg, (caddr_t) NULL},
...
static argtype vars[] =
{
  ...
  {(caddr_t *) &patterntrace, (char *) "patterntrace",
   (char *) "PatternTrace", (char *) DEF_PATTERNTRACE, t_Int},
...
static OptionStruct desc[] =
{
  ...
  {(char *) "-patterntrace num", (char *) "Trace Juggling Patterns"},
  {(char *) "-/+solid", (char *) "Trace Juggling Patterns"},
};
...
#define INSERT_AFTER(t, ot)					\
  if ((t = (Trajectory *)calloc(1, sizeof(Trajectory))) == NULL) \
    (void) fprintf(stderr, "Memory problems in juggle mode\n");	\
  (t)->next = (ot)->next;					\
  (t)->prev = (ot);						\
  (ot)->next = (t);						\
  (t)->next->prev = (t)
...
draw_figure(ModeInfo * mi)
{
  ...
  XSetForeground(dpy, gc, MI_WHITE_PIXEL(mi));
  XDrawLines(dpy, win, gc, sp->figure_path, FIGURE1,
			 CoordModeOrigin);
...
init_juggle(ModeInfo * mi)
{
	...
	if (pattern == NULL && patternindex == NULL) {
	  ...
	  /* allocate index */
	  if ((patternindex =
		   (PatternIndex *) calloc(maxballs + 1,
				sizeof (PatternIndex))) == NULL) {
    			(void) fprintf(stderr,
			  "Memory problems for patternindex in juggle mode\n");
		return;
	  }
	  ...
	  for (i = 0; i < nelements; i++) {
		...
		if (b > maxballs) {
		  if (MI_IS_VERBOSE(mi)) {
			fprintf(stderr, "%d %d ball pattern%s\n",
	...
	if (juggles == NULL) { /* allocate jugglestruct */
		if ((juggles = (jugglestruct *) calloc(MI_NUM_SCREENS(mi),
    		  	...
    		  	(void) fprintf(stderr,
			  "Memory problems for jugglestruct in juggle mode\n");
			return;
		}
	}
	...
	if (sp->trace == NULL) {
	  sp->trace = (XPoint *)calloc(patterntrace, sizeof(XPoint));
	}
...
#define POSITION_BIAS 20 /* larger makes hand movements less likely */
	  ...
	  program(mi, pattern, MI_CYCLES(mi));
	}
...
static void
draw_juggle_ball(ModeInfo *mi, long color, int x, int y, int degree_offset, int divisions)
{
	Display    *dpy = MI_DISPLAY(mi);
	...
	if (color == MI_WHITE_PIXEL(mi) || (divisions != 2 && divisions != 4) || sp->solid) {
		XFillArc(dpy, window, gc,
			x - BALLRADIUS, y - BALLRADIUS,
