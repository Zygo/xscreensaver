/* -*- Mode: C; tab-width: 4 -*- */
/* hyper --- spinning hypercubes, not just tesseracts any more */

#if !defined( lint ) && !defined( SABER )
static const char sccsid[] = "@(#)hyper.c 4.11 98/05/27 xlockmore";
...
static void
figure_points(ModeInfo * mi, hyperstruct * hp)
{
	int         i, j, k, n, d, pix = 0;
	int        *c;
...
static void
init_x_stuff(ModeInfo * mi, hyperstruct * hp)
{
	XGCValues   gcv;
	int         options;
	...
	if (!hp->xorGC) {
		...
		gcv.function = GXxor;
		hp->xorGC = XCreateGC(MI_DISPLAY(mi), MI_WINDOW(mi), options, &gcv);
	}
}

static void
move_line(ModeInfo * mi, int from, int to, int set, int color)
{
	...
	if (MI_NPIXELS(mi) <= 2) {
		...
		XSetForeground(display, hp->xorGC, color);
		if (!hp->redrawing)
			XDrawLine(display, window, hp->xorGC,
			hp->xpoints[!set][from].x, hp->xpoints[!set][from].y,
			   hp->xpoints[!set][to].x, hp->xpoints[!set][to].y);
		XDrawLine(display, window, hp->xorGC,
			  hp->xpoints[set][from].x, hp->xpoints[set][from].y,
...
static void
move_number(ModeInfo * mi, int axis, char *string, int set, int color)
{
	Display    *display = MI_DISPLAY(mi);
	Window      window = MI_WINDOW(mi);
	GC          gc = MI_GC(mi);
	...
	if (MI_NPIXELS(mi) <= 2) {
		...
		XSetForeground(display, hp->xorGC, color);
		if (!hp->redrawing)
			XDrawString(display, window, hp->xorGC,
			hp->xpoints[!set][axis].x, hp->xpoints[!set][axis].y,
				    string, strlen(string));
		XDrawString(display, window, hp->xorGC,
			  hp->xpoints[set][axis].x, hp->xpoints[set][axis].y,
...
static void
draw_hyper_step(ModeInfo * mi, hyperstruct * hp, int set)
{
	int         i;
	char        tmps[3];
...
static void
move_hyper(ModeInfo * mi, hyperstruct * hp)
{
	int         i;
...
static void
calc_transformation(hyperstruct * hp)
{
	double      cosa, sina;
	int         p1, p2;
	matrix     *Ttmp;
...
static void
translate_point(hyperstruct * hp, vector * real, XPoint * screen_image)
{
	vector     *image;
...
static void
translate_points(hyperstruct * hp, int set)
{
	int         i;
...
refresh_hyper(ModeInfo * mi)
{
	...
	MI_CLEARWINDOW(mi);
	hp->redrawing = True;
	draw_hyper_step(mi, hp, hp->this_set);
	hp->redrawing = False;
	hp->painted = True;
	}
}
