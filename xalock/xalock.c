...
static TagList timeopt[] =
{
  ...
  {TAG_INT, "nice level for xlock process:", &nice, TAG_INIT},
...
launch (Widget w, void *data)
{
  ...
  (void) sprintf (tmp, "xlock %s ", (char *) data);
...
/*if (strlen(namegetprog)>0) strcat2(strcommand,"%s", namegetprog) ; */
  ...
  if (nice != 0)
    strcatint2str (strcommand, "-nice ", nice);
  if (timeout != 0)
...
init_display (int argc, char **argv, void *data)
{
  ...
  w[10] = MakeScrollList ((char **) LockProcs, 110, 350, NULL, NULL);
  ListMode = w[10];
  ...
  SetWidgetPos (w[9], PLACE_RIGHT, w[0], NO_CARE, NULL);
  SetWidgetPos (w[10], PLACE_UNDER, w[0], NO_CARE, NULL);
  SetWidgetPos (w[8], PLACE_UNDER, w[10], NO_CARE, NULL);
  SetWidgetPos (w[12], PLACE_RIGHT, w[8], PLACE_UNDER, w[10]);

  SetWidgetPos (tog[0], PLACE_RIGHT, w[10], PLACE_UNDER, w[9]);
  ...
  for (i = 1; i < NBTOGGLE; i++)
    {
      SetWidgetPos (tog[i], PLACE_RIGHT, w[10], PLACE_UNDER, tog[i - 1]);
