...
/* Feb 2026: This version slightly modified by jwz to compile without        */
/* warnings with -DTRILIBRARY -DREDUCED -DCDT_ONLY -DANSI_DECLARATORS        */
/* and -std=gnu99 -pedantic -Wall -Wstrict-prototypes -Wmissing-prototypes   */
/* and to only export the symbols declared in triangle.h.                    */


/* jwz: for xscreensaver.
   It would obviously be better if we just passed these from the Makefile
   when building triangle.o, but it seems that Android's Gradle has *no*
   mechanism for passing different -D options to different .o files.
   Every .o in the same project has to have the same options.
 */
#define TRILIBRARY
#define REDUCED
#define CDT_ONLY
...
#define STATIC static   /* jwz: don't export internal symbols */
...
STATIC vertex triorg;                       /* The triangle's origin vertex. */
STATIC vertex tridest;                 /* The triangle's destination vertex. */
STATIC vertex triapex;                        /* The triangle's apex vertex. */
STATIC REAL area;                               /* The area of the triangle. */
...
#if (__GNUC__ >= 4)
      /* jwz: "ISO C forbids conversion of function pointer to object pointer type" */
...
#ifdef ANSI_DECLARATORS
STATIC char *readline(char *string, FILE *infile, char *infilename)
#else /* not ANSI_DECLARATORS */
